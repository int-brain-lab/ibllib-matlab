    ...
    methods
        ...
        function s = to_struct(self, ind)
            ...
            s = struct('coronal_index', self.coronal_index(ind),...
                'sagittal_index', self.sagittal_index(ind),...
                'index', self.index(ind),...
                'ap_um', round(self.dvmlap_entry(ind,3)*1e6),... % units mm, this rounding gets 2 decimal places in the output
                'ml_um', round(self.dvmlap_entry(ind,2)*1e6),...
                'dv_um', -round(self.dvmlap_entry(ind,1)*1e6),...
                'depth_um', round(self.depth(ind)*1e6),...
                'theta', round(self.theta(ind)*10)/10,...
                'phi', round(self.phi(ind)*10)/10);
        end
